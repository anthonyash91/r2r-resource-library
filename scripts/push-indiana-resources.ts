/**
 * Upsert Indiana resources from data/indiana-resources.csv into Supabase.
 * Requires SUPABASE_SERVICE_ROLE_KEY in .env.local (admin RLS bypass).
 *
 * Usage: npm run db:push:indiana
 */
...
const CSV_PATH = resolve(process.cwd(), "data/indiana-resources.csv");
const UUID_PREFIX = "d3000001";
const BATCH_SIZE = 50;
...
async function main() {
  ...
  if (!url || !serviceKey) {
    console.error(
      "Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY.\n" +
        "Add SUPABASE_SERVICE_ROLE_KEY to .env.local, or run supabase/seed-indiana-resources.sql in the SQL Editor."
    );
    process.exit(1);
  }
  ...
  const { count } = await supabase
    ...
    .eq("state", "Indiana");
  console.log(`Done. Indiana resources in database: ${count ?? upserted}`);
}
