/**
 * Upsert Kentucky resources from data/kentucky-resources.csv into Supabase.
 * Requires SUPABASE_SERVICE_ROLE_KEY in .env.local (admin RLS bypass).
 *
 * Usage: npm run db:push:kentucky
 */
import { readFileSync, existsSync } from "fs";
import { resolve } from "path";
import { createClient } from "@supabase/supabase-js";
import { isValidCoverage } from "../src/lib/resource-coverage";
import { resolveCategoryId } from "./resource-category-slugs";
...
async function main() {
  ...
  if (!url || !serviceKey) {
    console.error(
      "Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY.\n" +
        "Add SUPABASE_SERVICE_ROLE_KEY to .env.local, or run supabase/seed-kentucky-resources.sql in the SQL Editor."
    );
    process.exit(1);
  }
  ...
  const payloads = records.map((row, index) => {
    ...
    if (!categoryId) {
      throw new Error(`Unknown category "${categoryRaw}" on row ${index + 2}`);
    }
  ...
  const { count } = await supabase
    ...
    .eq("state", "Kentucky");
  console.log(`Done. Kentucky resources in database: ${count ?? upserted}`);
}
