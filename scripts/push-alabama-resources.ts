/**
 * Upsert Alabama resources from data/alabama-resources.csv into Supabase.
 * Requires SUPABASE_SERVICE_ROLE_KEY in .env.local (admin RLS bypass).
 *
 * Usage: npm run db:push:alabama
 */
...
const CSV_PATH = resolve(process.cwd(), "data/alabama-resources.csv");
const UUID_PREFIX = "dc000001";
const BATCH_SIZE = 50;
...
async function main() {
  ...
  if (!url || !serviceKey) {
    console.error(
      "Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY.\n" +
        "Add SUPABASE_SERVICE_ROLE_KEY to .env.local, or run supabase/seed-alabama-resources.sql in the SQL Editor."
    );
    process.exit(1);
  }
  ...
  const { count } = await supabase
    ...
    .eq("state", "Alabama");
  console.log(`Done. Alabama resources in database: ${count ?? upserted}`);
}
