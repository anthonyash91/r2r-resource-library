/**
 * Upsert West Virginia resources from data/west-virginia-resources.csv into Supabase.
 * Requires SUPABASE_SERVICE_ROLE_KEY in .env.local (admin RLS bypass).
 *
 * Usage: npm run db:push:west-virginia
 */
...
const CSV_PATH = resolve(process.cwd(), "data/west-virginia-resources.csv");
const UUID_PREFIX = "d7000001";
const BATCH_SIZE = 50;
...
async function main() {
  ...
  if (!url || !serviceKey) {
    console.error(
      "Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY.\n" +
        "Add SUPABASE_SERVICE_ROLE_KEY to .env.local, or run supabase/seed-west-virginia-resources.sql in the SQL Editor."
    );
    process.exit(1);
  }
  ...
  const { count } = await supabase
    ...
    .eq("state", "West Virginia");
  console.log(`Done. West Virginia resources in database: ${count ?? upserted}`);
}
