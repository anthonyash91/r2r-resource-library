/**
 * Upsert Arizona resources from data/arizona-resources.csv into Supabase.
 * Requires SUPABASE_SERVICE_ROLE_KEY in .env.local (admin RLS bypass).
 *
 * Usage: npm run db:push:arizona
 */
...
const CSV_PATH = resolve(process.cwd(), "data/arizona-resources.csv");
const UUID_PREFIX = "dd000001";
const BATCH_SIZE = 50;
...
async function main() {
  ...
  if (!url || !serviceKey) {
    console.error(
      "Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY.\n" +
        "Add SUPABASE_SERVICE_ROLE_KEY to .env.local, or run supabase/seed-arizona-resources.sql in the SQL Editor."
    );
    process.exit(1);
  }
  ...
  const { count } = await supabase
    ...
    .eq("state", "Arizona");
  console.log(`Done. Arizona resources in database: ${count ?? upserted}`);
}
