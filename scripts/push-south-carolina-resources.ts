/**
 * Upsert South Carolina resources from data/south-carolina-resources.csv into Supabase.
 * Requires SUPABASE_SERVICE_ROLE_KEY in .env.local (admin RLS bypass).
 *
 * Usage: npm run db:push:south-carolina
 */
...
const CSV_PATH = resolve(process.cwd(), "data/south-carolina-resources.csv");
const UUID_PREFIX = "db000001";
const BATCH_SIZE = 50;
...
async function main() {
  ...
  if (!url || !serviceKey) {
    console.error(
      "Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY.\n" +
        "Add SUPABASE_SERVICE_ROLE_KEY to .env.local, or run supabase/seed-south-carolina-resources.sql in the SQL Editor."
    );
    process.exit(1);
  }
  ...
  console.log(`Done. South Carolina resources in database: ${count ?? upserted}`);
}
